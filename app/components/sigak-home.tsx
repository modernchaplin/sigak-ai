  ...
  return (
    <section className="sigak-mvp -mx-4 -my-8 min-h-[calc(100vh-4rem)] px-4 py-10 text-white md:px-6 md:py-14">
      <div className="mx-auto max-w-6xl">
        ...
        {viewMode === "home" && (
          <section className="mt-8 rounded-2xl border border-zinc-800 bg-zinc-950/55 p-5 md:p-7">
            <p className="mono text-xs tracking-[0.2em] text-yellow-300">탐색 방식</p>
            <h2 className="mt-3 text-2xl font-semibold md:text-3xl">
              먼저 하나의 축을 고르면, 그 아래 콘텐츠 입구가 열립니다.
            </h2>
            <p className="mt-4 max-w-2xl text-sm leading-7 text-zinc-400">
              SIGAK.AI는 정보를 한 번에 펼쳐두기보다 시간, 사람, 지도, 기록, 시각이라는
              다섯 관점으로 나누어 AI 시대를 읽습니다.
            </p>
          </section>
        )}
    ...
    <section className="sigak-mvp -mx-4 -my-8 min-h-[calc(100vh-4rem)] px-4 py-10 text-white md:px-6 md:py-14">
      <article className="mx-auto max-w-5xl">
        ...
        <div className="mt-8 rounded-3xl border border-zinc-800 bg-zinc-950 px-5 py-7 md:px-8 md:py-10">
          <p className="mono text-xs tracking-[0.2em] text-yellow-300">{topic.axis}</p>
        ...
        {topic.timeline && topic.timeline.length > 0 && <TimelineBlock items={topic.timeline} />}
...
function getDetailSections(topic: SigakTopic) {
  if (topic.id === "agi-timeline") {
    return [
      ...
      { title: "SIGAK.AI의 시각", body: topic.sections.sigak, emphasized: true },
